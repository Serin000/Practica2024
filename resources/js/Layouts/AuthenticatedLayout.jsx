...
export default function Authenticated({children, flash}) {
    ...
    return (<div className="flex flex-row relative">
        <aside className={`min-h-screen w-1/6 bg-gray-800 text-white flex flex-col`}>
            <Link className="flex justify-center py-2 mb-4" href="/">
                <ApplicationLogo className="w-10 h-10 fill-current text-white"/>
            </Link>
            <Link
                href={route('dashboard')}
                className="w-56 py-2 px-3 mx-auto text-white rounded hover:bg-gray-700 mb-2">
                Dashboard
            </Link>
            <Link
            ...
            <form onSubmit={submit}>
                <button
                    className="block w-56 py-2 px-3 mx-auto text-white rounded hover:bg-gray-700 mb-6">
                    Logout
                </button>
            </form>
        </aside>
